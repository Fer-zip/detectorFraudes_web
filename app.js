...
// Controles para mostrar resultads
...
// ----------- MODELO ML ----
const URL = "https://teachablemachine.withgoogle.com/models/MWRkWEW6Y/";
...
async function init() {
    ...
    maxPredictions = model.getTotalClasses();
        ...
        Swal.fire({
            icon: "error",
            title: "Error",
            text: "No se pudo cargar el modelo de Teachable Machine. Por favor, intenta más tarde."
        });
    }
}
...
// --- NUEVO: Función para predecir usando la imagen cargada ---
async function predictImage() {
    try{
        ...
        return prediction[0]; // Retorna la predicción más probable
    }catch (error) {
        console.error("Error al predecir la imagen:", error);
        Swal.fire({
            icon: "error",
            title: "Error",
            text: "No se pudo analizar la imagen. Por favor, intenta con otra imagen."
        });
        return null;
    }
}

// --- MODIFICA el evento del botón Analizar Imagen ---
btnAnalizarImage.addEventListener('click', async ()=>{
    try{
        const previewImg = document.querySelector('.previewImage img');
        if (previewImg && inputImage.files.length > 0) {
            Swal.fire({
                title: "Analizando imagen...",
                text: "Espera mientras tanto se analiza la imagen...",
                icon: "success"
            });
            imgresultPRE && imgresultPRE.remove();
            const result = await predictImage();
            if (result) {
                let icono = result.className.toLowerCase().includes('fraude') ? alertaFraude : noFraude;
                previewResult.innerHTML = icono;
                textResult.innerHTML = `La imagen es clasificada como <b>${result.className}</b> con ${(result.probability*100).toFixed(2)}% de confianza.`;
            } else {
                previewResult.innerHTML = '';
                textResult.innerHTML = 'No se pudo analizar la imagen.';
            }
        } else {
            Swal.fire({
                icon: "error",
...
// PROCESAMIENTO PARA EL TEXTO
const textArea = document.querySelector('#text')
const URLBACKEND = 'https://backend-flask-ml-correos.onrender.com/predict';

btnAnalizarText.addEventListener('click', async ()=>{
    if (textArea.value) {
        Swal.fire({
            title: "Analizando texto...",
            text: "Esperemos mientras se analiza tu correo...",
            icon: "success"
        });
        previewResult.innerHTML = ""; // Limpia el icono anterior
        textResult.innerHTML = "Analizando texto...";
        ...
        try {
            ...
            // Ejemplo: data.result = "Fraude" o "Legitimo", data.probabilidad = 0.95
            let icono = "";
            if (data.etiqueta_clase && data.etiqueta_clase.toLowerCase().includes("frau")) {
                icono = alertaFraude;
            } else if (data.etiqueta_clase && data.etiqueta_clase.toLowerCase().includes("legit")) {
                icono = noFraude;
            } else {
                icono = posibleFraude;
            }
            ...
            previewResult.innerHTML = icono;
            textResult.innerHTML = `El texto es clasificado como <b>${data.etiqueta_clase}</b> con ${(data.porcentaje_clase)}% de confianza.`;

        } catch (error) {
            previewResult.innerHTML = "";
            console.error("Error al analizar el texto:", error);
            textResult.innerHTML = "No se pudo analizar el texto. Intenta de nuevo.";
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "No se pudo analizar el texto. Intenta de nuevo."
            });
        }
    } else {
        Swal.fire({
...
inputImage.addEventListener('change', function(event) {
    ...
    if (file && file.type.startsWith('image/')) {
        // Elimina el SVG inicial si existe
        const svg = document.getElementById('change_svg');
        if (svg) svg.remove();
        // Muestra la imagen seleccionada
        const reader = new FileReader();
        reader.onload = function(e) {
            previewContainer.innerHTML = `<img src="${e.target.result}" alt="Vista previa de la imagen" style="max-width:100%;max-height:400px;">`;
        };
